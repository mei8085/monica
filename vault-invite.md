...
| **前端限制可绕过** | 前端 `resources/js/Pages/Vault/Settings/Partials/Users.vue` 通过单选按钮硬编码 value="100"/"200"/"300"，但直接调用 API 可绕过 | API 层面无防护，恶意调用可能越权 |
...
在 `resources/js/Pages/Vault/Settings/Partials/Users.vue` 的 `store()` 方法第 288-292 行：
...
`tests/Unit/Domains/Vault/ManageVaultSettings/Services/GrantVaultAccessToUserTest.php` 中没有覆盖"同一用户重复授权"的场景，测试用例只验证了正常授权流程。
...
**行为**：`first()` 返回匹配记录中的任意一条。查询没有显式 `orderBy`，数据库不保证返回顺序，因此无法确定更新的是哪条记录。

**影响**：
- 如果存在多条记录，只有其中一条的 permission 会被更新，其余保持原值不变
- 由于无法确定更新的是哪条，降低权限（如 100 → 300）后，如果另一条记录仍为 100，则用户实际权限仍然是 MANAGE
- **前端显示可能与实际权限不符**
...
**行为**：`first()` 返回匹配记录中的任意一条。查询没有显式 `orderBy`，数据库不保证返回顺序，因此无法确定删除的是哪条记录。删除其关联的 Contact 后，外键级联会删除对应的 `user_vault` 记录。

**影响**：
- 如果存在多条记录，只有其中一条被删除，其余仍然存在
- 用户权限不会因单条记录删除而丧失
- **出现"已移除但仍有权限"的不一致状态**
